...
class Dataset:
    def __init__(self, train_list, test_list, database_root, store_memory=True, data_aug=False):
        ...
        for idx, line in enumerate(train_paths):
            if store_memory:
                ...
                cls_label = int(line.split('\t')[2])
                if data_aug:
                    for angle in data_aug_rotates:
                        img_rot = img.rotate(angle)
                        label_rot = label.rotate(angle)
                        self.images_train.append(np.array(img_rot, dtype=np.uint8))
                        self.labels_train.append(np.array(label_rot, dtype=np.uint8))
                        self.cls_labels_train.append(cls_label)
                        if data_aug_flip:
                            img_rot_fl = img_rot.transpose(Image.FLIP_LEFT_RIGHT)
                            label_rot_fl = label_rot.transpose(Image.FLIP_LEFT_RIGHT)
                            self.images_train.append(np.array(img_rot_fl, dtype=np.uint8))
                            self.labels_train.append(np.array(label_rot_fl, dtype=np.uint8))
                            self.cls_labels_train.append(cls_label)
                else:
        ...
        for idx, line in enumerate(test_paths):
            if store_memory:
                self.images_test.append(np.array(Image.open(os.path.join(database_root, str(line.split('\t')[0]))), dtype=np.uint8))
                if (idx + 1) % 1000 == 0:
    ...
    def next_batch(self, batch_size, phase):
        ...
        if phase == 'train':
            ...
            return images, labels, cls_labels
        elif phase == 'test':
            images = None
