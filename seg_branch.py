...
def _train(dataset, valid_dataset, num_classes, initial_ckpt, supervison, learning_rate, logs_path, max_training_iters, save_step, display_step,
    ...
    with slim.arg_scope(osvos_arg_scope()):
        net, end_points = osvos(input_image, num_classes)
    ...
    with tf.Session(config=config) as sess:
        ...
        while step < max_training_iters + 1:
            ...
            for _ in range(0, iter_mean_grad):
                ...
                batch_image, batch_label, _ = dataset.next_batch(batch_size, 'train')
            ...
            # LIAO: validation
            if step % test_step == 0:
                valid_image, valid_label, _ = valid_dataset.next_batch(batch_size, 'train')
                for i in range(batch_size):
                   image = valid_image[i]
                   if type(image) is not np.ndarray:
                        image = np.array(Image.open(image), dtype=np.uint8)
                   ...
                   max_mask = np.max(label) * 0.5
                   label = np.greater(label, max_mask)
                   valid_label[i] = np.expand_dims(label, axis=3)

                valid_res = sess.run([total_loss, merged_summary_op],
                                    feed_dict={input_image: valid_image, input_label: valid_label})
